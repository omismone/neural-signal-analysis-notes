...
Network Burst, as it involves all the channels. On the Y axis are represented the labels of the electrodes.
...
A task-related raster plot visualizes neural activity from a single electrode, aligned to a specific event (e.g., stimulus onset). The time axis represents time relative to the stimulus, centering all trials around it. Each row corresponds to a single trial, showing how the neuron responds across repeated presentations of the same stimulus.
...
\subsection{Spike Count and Firing Rate metrics}
...
\end{equation*}
\begin{figure}[H]
    \includegraphics[scale=0.4]{4_9}
    \centering
\end{figure}
EC is the effective concentration.
\subsection{Inter Spike Interval}
