...
\textbf{extracellular} space around neurons. Its acquisition requires invasive electrodes
...
The \textit{local} term is misleading, as it is meant to indicate that
...
\textit{Cz} on the top of the head (above HC). The correlation is not anymore as good as before.
\begin{figure}[H]
...
is suppressed as soon as the reaching movement is completed. \\ 
...
to the \textit{forward problem} is by exploiting \textbf{compartmental models} of the neurons (soma, dendrites, axon..), then
...
correlation patterns for the synaptic inputs and, on the other side, \textbf{the spatial
...
These bands are often called \textbf{Berger Bands}. They are far from being accurate
