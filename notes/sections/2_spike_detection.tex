...
\begin{itemize}
    ...
    \item \(>150\,\mu{m}\): spikes cannot be detected, they contribute to the noise.
\end{itemize}
\begin{figure}[H]
...
\begin{itemize}
    ...
    \item \textbf{Spike detection:} always keeps the same format (matrix)
...
Let's recall the mathematical definition of a spike train:
\begin{itemize}
...
Simple thresholding is characterized by a very simple logic and for this reason it is often employed in
commercial systems for data acquisition where the online detection of the spikes is required. This kind of detection technique is often useful because it is quite fast, even if at the expenses of precision.
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{2_7.png}
\end{figure}

Notice this algorithm is applied also to absolute-valued
...
\end{itemize}
MAD = Median Absolute Deviation is a robust statistical measure. It is computed as \(MAD=median(|x-median(x)|)\), it works well in situations where the data contains large deviations.\\ 
A common way to approximate the standard deviation using MAD is: $\sigma \approx 1.4826 \cdot MAD$. Since MAD is based on medians rather than means, it is resistant to outliers, unlike the standard deviation, which can be heavily influenced by extreme values.
\paragraph{Hard Threshold - Local Maxima version}
In this case the spike position is associated to the local maxima (and not set at the time of the first sample overcoming the threshold).
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{2_8.png}
\end{figure}
\paragraph{Differential Threshold}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{2_9.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{2_10.png}
\end{figure}
...
\begin{equation*}
    SE(i)=\frac{1}{W}\sum_{j=i-\frac{W}{2}}^{i+\frac{W}{2}}V(j)^2
\end{equation*}
...
This transformation produces a new signal that enhances the local energy fluctuations while preserving the sign of the original signal. By doing so, SE(i) highlights potential spike events, making them more distinguishable from background noise. Traditional spike detection methods, such as thresholding and peak detection, can then be applied to this processed signal, improving robustness and reducing false detections.
...
\begin{itemize}
    \item \textbf{Experimental groundtruth}: neurons are recorded into two distinct ways,
          ...
          precise intracellular recordings (exhibiting precise timing and localization) to find
    ...
    \item \textbf{Computational groundtruth}: data are synthetic and generated by a model
          reproducing the behavior of a network of neurons under different conditions.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{2_11}
\end{figure}
\begin{enumerate}
...
\paragraph{Confusion Matrix} This matrix is special type of contingency matrix, and
it allows visualizing the performance of an algorithm. Each row of the matrix
...
such a visualization allows assessing whether the system is confusing two classes.
\begin{figure}[H]
