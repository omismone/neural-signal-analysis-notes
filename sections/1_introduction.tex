...
\begin{itemize}
      ...
      \item \textbf{Plasticity:} certain permanent functional transformations
      arise in particular systems of neurons as a
      result of appropriate stimuli or their
      combination.
\end{itemize}
Action potentials are rapid, all-or-none electrical impulses that neurons use to transmit information. When a neuron is stimulated beyond a critical threshold, its membrane quickly depolarizes (the inside becomes less negative compared to the outside), mainly due to sodium influx, and then repolarizes (as potassium exits), sending a signal along the axon.\\ 

They are a self-propagating electrical event triggered when a neuron's membrane potential reaches a critical threshold. At this point, voltage-gated sodium channels open, allowing a rapid influx of ${Na}^+$ ions that depolarizes the membrane. This depolarization initiates a wave along the axon, ensuring that each adjacent segment of the membrane reaches threshold in an all-or-none fashion. Shortly after, the sodium channels inactivate and voltage-gated potassium channels open, leading to an efflux of $K^+$ ions that repolarizes—and often briefly hyperpolarizes—the membrane before it returns to its resting state. This precisely timed sequence enables neurons to reliably and rapidly transmit signals over long distances in the nervous system.
