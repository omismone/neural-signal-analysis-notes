...
\begin{itemize}
      ...
      \item \textbf{Plasticity:} the brain’s ability to undergo lasting microstructural and functional changes (e.g. synaptic strengthening, dendritic spine remodeling) in response to electrical or chemical stimuli.
\end{itemize}

\subsection{Action Potentials}

When a neuron is at rest, its membrane potential is approximately \(-70\,\mathrm{mV}\), maintained by the unequal distribution of sodium and potassium ions across the membrane. Sodium concentration is higher outside the cell, potassium concentration is higher inside. This ionic separation generates both a chemical gradient and, because ions are charged, an electrical gradient. Together these form the electrochemical gradient, and the resulting difference in charge across the membrane is the resting membrane potential.\\

Ion movement occurs through specialized channels embedded in the membrane. Voltage‑gated channels open or close in response to changes in membrane potential; ligand‑gated channels open when specific molecules bind; mechanically‑gated channels respond to physical forces such as stretch or pressure. Each channel type is selectively permeable, ensuring that only particular ions cross under appropriate conditions, thus finely tuning membrane dynamics.\\

Upon sufficient stimulation—when depolarization raises the membrane potential from \(-70\,\mathrm{mV}\) to the threshold of about \(-55\,\mathrm{mV}\)—voltage‑gated sodium channels open. Sodium ions rush into the cell along their electrochemical gradient, causing a rapid depolarization that drives the membrane potential toward \(0\,\mathrm{mV}\) and then to an overshoot of approximately \(+30\,\mathrm{mV}\). At this peak, the inactivation gates of the sodium channels close, halting further Na\(^+\) influx, while voltage‑gated potassium channels begin to open.\\

As potassium channels open more slowly, K\(^+\) efflux repolarizes the membrane, bringing the potential back down toward the resting level. Because these channels close slowly, the membrane briefly hyperpolarizes—falling below the resting potential—before returning to \(-70\,\mathrm{mV}\). The Na\(^+\)/K\(^+\)-ATPase pump uses ATP hydrolysis to transport three Na\(^+\) ions out and two K\(^+\) ions in, correcting the ionic imbalances and contributing to return from hyperpolarization to the resting state.\\

During and immediately after an action potential, the neuron enters refractory periods that govern signal timing. The absolute refractory period—while sodium channels remain inactivated—prevents any new action potential regardless of stimulus strength. This is followed by the relative refractory period, during which sodium channels reset but the membrane is hyperpolarized, so a larger-than-normal stimulus is required to elicit another action potential.\\

Once initiated at the axon hillock, the action potential propagates along the axon by sequential depolarization of adjacent membrane segments. Myelin sheaths—formed by Schwann cells in the PNS and oligodendrocytes in the CNS—insulate the axon, forcing ionic currents to jump between exposed Nodes of Ranvier in saltatory conduction. This mechanism greatly increases conduction velocity and preserves signal fidelity over long distances.\\


This precisely timed sequence enables neurons to reliably and rapidly transmit signals over long distances in the nervous system.
...
This is what neuroengineering is actually about. 
It can be described as a tale of two loops: from one point of view, we have the development of new technologies from the knowledge of the brain, while from the other one, new technologies can be useful to treat the brain or the nervous system.\\
...
\begin{itemize}
      ...
      \item \textbf{Depth:} electrodes placed in subcortical areas.
\end{itemize}
...
\begin{figure}[H]
      ...
      \centering
\end{figure}
The multiscale brain model presented above aims at showing the different
...
interest is time.\\
Ensembles of spike trains from simultaneously recorded neurons are multidimensional point-process time series. These time series are both dynamic
and stochastic.
\begin{itemize}
