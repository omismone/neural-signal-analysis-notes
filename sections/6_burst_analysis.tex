...
\end{figure}
\noindent
\textbf{Practical example.} To better understand how the cross-correlogram works, let's consider a very simple case with two binary spike trains, where 1 indicates the presence of a spike and 0 its absence. Each bin corresponds to 1 ms:

\begin{itemize}
	\item Reference train \(x\): [0, 1, 0, 0, 1, 0, 0, 0, 1] \hfill (spikes at \(t = 1, 4, 8\))
	\item Target train \(y\): [0, 0, 1, 0, 1, 0, 1, 0, 0] \hfill (spikes at \(t = 2, 4, 6\))
\end{itemize}

We consider a symmetric time window around each spike in the reference train, from \(\tau = -2\) to \(\tau = +2\) ms. For each spike in the reference train, we compute the relative delay \(\tau = t_y - t_x\) for the spikes in the target train that fall within the time window. Each \(\tau\) is then binned to build the cross-correlogram histogram.

\begin{enumerate}
	\item Reference spike at \(t = 1\): target spike at \(t = 2 \rightarrow \tau = +1\)
	\item Reference spike at \(t = 4\): target spikes at \(t = 2, 4, 6 \rightarrow \tau = -2, 0, +2\)
	\item Reference spike at \(t = 8\): target spike at \(t = 6 \rightarrow \tau = -2\)
\end{enumerate}
...
\noindent
This example illustrates how temporal alignment between the two trains leads to peaks in the cross-correlogram. In particular, the peak at \(\tau = -2\) suggests that the target neuron often fires about 2 ms before the reference. The presence of a central peak at \(\tau = 0\) also indicates at least one synchronized firing event.
